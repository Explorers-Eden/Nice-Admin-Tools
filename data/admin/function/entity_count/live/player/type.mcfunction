$execute at $(player) as @e[type=$(mobtype),distance=..$(distance)] if data storage eden:temp admin.live_count{glowing:1} run effect give @s minecraft:glowing $(glow_duration) 1 true
$execute at $(player) as @e[type=$(mobtype),distance=..$(distance)] run scoreboard players add $entity_count admin.technical 1
$tellraw @s [\
    {"color":"gold","text":"▊ "},\
